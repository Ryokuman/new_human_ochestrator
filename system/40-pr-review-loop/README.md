...
- Codex 응답은 사용자 응답을 기다리지 않고 확인합니다. 호출 뒤 3분 동안 `eyes` 반응이 없으면 접수 실패로 보고 같은 head 기준으로 최대 3회까지 재호출한 뒤 새 호출 댓글 기준으로 다시 확인합니다. 3회 모두 접수되지 않으면 `Codex 리뷰 접수 실패 timeout`으로 중단합니다. `eyes` 반응을 확인한 뒤 15분 동안 응답이 없으면 `Codex 리뷰 응답 대기 timeout`으로 중단합니다. no-major 응답이 아니면 타당한 지적을 수정, 검증, push한 뒤 재리뷰를 요청합니다.
