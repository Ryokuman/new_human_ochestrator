...
- 사용자가 PR을 리뷰 대기 에이전트로 돌리라고 하면 `review-waiter-agent`가 승인 리뷰까지 백그라운드 루프로 관리합니다. task silo의 `goal.md`가 확인되면 `/goal`을 재사용하고, 그렇지 않으면 PR 본문, 리뷰 thread, 현재 사용자 요청을 컨텍스트로 사용합니다.
