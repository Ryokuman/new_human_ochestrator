...
기능 task를 바로 사일로 구현으로 넘기지 않고, 먼저 단계별 구현 계획과 pseudo code를 작성하게 한 뒤 리뷰하면 목표 밖 화면, API, DB mutation, submodule, E2E 범위 drift를 구현 전에 발견할 수 있다.

## 채택 이유

이전 방식은 task를 밀어넣은 뒤 구현 결과와 화면을 보고 나서야 drift가 드러났다. 그러면 이미 PR, submodule, harness, 사용자 확인이 얽힌 뒤라 폐기 비용이 커졌다.

계획과 pseudo code를 먼저 리뷰하면 실제 코드를 쓰기 전에 구현 범위, 데이터 흐름, 화면 상태 변화, 검증 경로를 비교할 수 있다.
...
- 계획 누락이 task-writer, main-orchestrator, goal template, PR 본문 중 어디에서 발생했는지 추적하기 어려웠다.
