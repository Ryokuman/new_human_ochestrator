...
- [`codex-pr-review-loop`](codex-pr-review-loop/SKILL.md): PR 유형을 계층별로 판정한 뒤 0계층 `main-v2` PR 또는 project 계층 PR에서 Codex가 최신 head에 대해 no-major 응답을 명시할 때까지 목표 세팅, 리뷰 호출, `eyes` 없음 3분 접수 대기, `eyes` 이후 15분 응답 대기, 수정, 검증, 재리뷰 반복을 관리할 때 사용합니다. submodule repo PR에서는 `patch/evidence`, `runtime`, `service/MSA` 유형과 제품 pin 조건을 함께 확인합니다.
