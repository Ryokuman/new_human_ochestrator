...
사용자가 `사용한 스킬`, 보기 3개, 다음 행동, evidence 기록 누락을 지적하면 원인을 아래 단계로 나눕니다.

1. `규칙 탐지 실패`: 이미 있는 규칙을 응답 시점에 떠올리지 못했습니다.
2. `상황 분류 실패`: 다음 행동이나 승인 경계가 남아 있는데 없다고 판단했습니다.
3. `최종 응답 체크 실패`: 답변 직전 `사용한 스킬`, `다음 행동`, `보기 밖 선택/evidence` 체크를 하지 않았습니다.
