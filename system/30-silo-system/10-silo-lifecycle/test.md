...
```text
1. 준비: goal, 대상, runtime 또는 runtime set, 금지선, report 위치를 명시한다.
2. 실행: lifecycle/E2E/runtime/탐색 검증을 수행하고 evidence를 수집한다.
3. 종료: 개별 보고서를 작성하고 evidence/보고서가 SSoT 또는 지정 위치로 승격됐는지 확인한다.
4. 삭제: 보고서 승격 gate와 dirty status 확인을 통과한 뒤 사일로 디렉토리를 삭제할 수 있다.
5. execution window 종료: 개별 보고서를 묶어 전체 사일로 보고서를 작성하고 반복 원인을 추출한 뒤 issue/task 생성 후보를 만든다.
```
