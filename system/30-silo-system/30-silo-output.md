...
- criteria별 검증 결과
- 리뷰 gate 결과 또는 생략 사유
- 2계층 `Project Work SSoT` 반영 대상: task, issue, QA, runbook, coverage, work dashboard, Run Set 중 해당 항목과 project SSoT 경로
...
테스트 사일로는 `Hypothesis Chain`을 쓰지 않습니다. 테스트 실패는 개별 보고서, test evidence, 전체 사일로 보고서, issue/task 승격 후보로만 정리합니다.
