...
- PR 생성 시 Codex gate는 먼저 PR 유형을 판정합니다. 0계층 공통 변경은 `main-v2`, project 계층 변경은 해당 `project/<project-id>` 대상으로 PR을 올린 뒤 `codex-pr-review-loop` skill로 no-major 목표를 세팅합니다. Codex 응답 대기, 수정, 검증, 재리뷰 반복을 맡길 실행자가 필요하면 `review-waiter-agent`를 사용합니다.
