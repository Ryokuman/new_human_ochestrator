...
6. 기능 task는 프론트/백엔드 분리 task가 아니라 사용자 목적과 완료 경로 기준의 풀스택 task로 작성합니다.
7. API, schema, store, route가 아직 없으면 위험으로 단정하지 않고, 같은 task 안에서 백엔드 계약을 먼저 만들고 프론트가 소비하는 실행 순서를 적습니다.
8. 사용자 화면, 설치, 서버 실행, 앱 다운로드 가능 상태가 acceptance에 포함되면 `Pre-QA Gate`와 사용자 QA 리스트를 적습니다.
9. runner, E2E, agent-browser, 외부 도구를 실행할 수 없을 가능성이 있으면 실행 불가 사유, 대체 증거, 남은 수동 확인 범위를 적습니다.
10. 사일로 실행이 필요하면 사일로 유형, 금지선, runtime set, branch 정책을 적습니다.
11. coverage 개선형 task는 초기 수치, 목표 수치, 기준 report, 남은 가설을 적습니다.
12. 실행 전제가 빠진 항목은 누락 정의로 분리합니다.
