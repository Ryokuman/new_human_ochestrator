...
- 사람 확인을 요청하기 전 agent가 직접 묶을 수 있는 test evidence를 먼저 모읍니다. 최소 단위는 criteria별 `test command`, `DB query`, `browser test evidence`, 실행 URL/명령, 로그, report 위치이며, 사람 확인은 최종 승인, UX 판단, 로컬 재현, 실제 계정/기기 접근처럼 agent가 증명할 수 없는 범위로 제한합니다.
