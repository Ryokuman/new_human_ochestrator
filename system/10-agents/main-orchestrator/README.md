...
`main-orchestrator-agent`는 사용자 요청을 계층, 사일로, skill, PR, feedback/follow-up 후보로 분류하고 전체 실행 흐름을 조율하는 역할입니다.

메인 오케스트레이터는 직접 모든 코드를 고치는 존재가 아닙니다. 전체 상태를 보고, 어떤 사일로가 어떤 일을 해야 하는지 결정하고, 결과를 리뷰하고, feedback/follow-up 후보를 정리합니다.
...
- 일반 사일로의 가설 체인과 테스트 사일로의 report/test evidence 흐름을 섞지 않습니다.
