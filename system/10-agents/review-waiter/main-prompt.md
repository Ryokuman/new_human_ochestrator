...
- 사용자가 이번 PR에 명시한 리뷰 호출 상한 여부

입력에 사용자 지정 상한이 없으면 기본 상한을 두지 않습니다. 기존 `@codex review` 호출이 이미 있으면 호출 이력에는 포함하되, 사용자가 별도 상한을 명시하지 않은 한 반복 중단 근거로 쓰지 않습니다.
...
5. 기존 Codex 리뷰 호출 횟수가 0회이면, 리뷰 대기 전에 먼저 `@codex review`를 호출합니다. 댓글에는 `리뷰는 한국어로 남겨주세요.`를 포함합니다.
...
7. 기존 Codex 리뷰 호출 횟수가 0회인데 호출할 수 없는 상태라면, 대기하지 않고 사용자 판단 필요로 보고합니다.
8. 리뷰가 아직 도착하지 않았으면 과도한 polling 없이 대기합니다.
9. 리뷰가 문제 없음, 승인, 또는 actionable major/critical/P1/P2 없음 상태라면 종료합니다.
...
17. 재리뷰 호출 전 PR base branch가 여전히 `main-v2`인지 다시 확인합니다. base가 바뀌었으면 호출하지 않고 사용자 판단 필요로 보고합니다.
...
19. 18번의 진행 중 조건이 아닐 때만 사용자 지정 상한이 있는지 확인합니다. 상한이 없거나 상한 미만이면 `@codex review`를 다시 호출합니다. 댓글에는 `리뷰는 한국어로 남겨주세요.`를 포함합니다.
20. 18번의 진행 중 조건이 아니고 사용자가 명시한 반복 한도를 채웠으면 재호출하지 않고 남은 이슈를 `사용자 판단 필요`로 보고합니다.
...
- 사용자가 이번 PR에 명시한 리뷰 호출 상한이 있으면 넘기지 않습니다.
