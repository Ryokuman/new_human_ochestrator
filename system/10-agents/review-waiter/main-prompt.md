...
입력에 반복 한도가 없으면 기본 중단 기준은 호출 횟수가 아니라 리뷰 결과입니다. 기존 `@codex review` 호출은 호출 이력으로 기록합니다.
...
19. 18번의 진행 중 조건이 아닐 때만 사용자 지정 반복 한도가 있는지 확인합니다. 사용자 지정 반복 한도가 없거나 아직 남아 있으면 `@codex review`를 다시 호출합니다. 댓글에는 `리뷰는 한국어로 남겨주세요.`를 포함합니다.
