...
- PR 생성 이후 승인 리뷰를 받을 때까지 반복 수정해야 할 때
...
- 기존 `@codex review` 호출이 0회이고 base branch가 `main-v2`이면, 리뷰 대기 전에 먼저 `@codex review`를 호출합니다.
...
- 기본 리뷰 호출 상한은 두지 않습니다. 사용자가 이번 PR에 별도 상한을 명시한 경우에만 그 상한을 따릅니다.
...
- 종료 시 리뷰 호출 횟수, 수정 커밋, 검증, 남은 위험, 사용자 지정 상한 적용 여부를 보고합니다.
...
- 사용자가 이번 PR에 명시한 리뷰 상한이 있으면 넘기지 않습니다.
