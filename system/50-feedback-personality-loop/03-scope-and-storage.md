...
| 사일로 전용 | Silo Local / Evidence | 이 사일로가 QA를 얕게 한 문제 |
...
3계층 Silo Local / Evidence는 현재 local 피드백 기록과 사일로 임시 evidence를 담는 계층입니다. 여기에는 아직 0계층 공통 규칙이나 1계층 project 기준으로 확정하지 않은 관찰, 보류 판단, follow-up을 둡니다.
...
- 반복 가능한 구조라면 0계층 `system/`의 템플릿이나 스킬 초안 반영 후보 evidence로 남깁니다.
...
응답 계약에 영향을 주는 사건은 장기 반영 여부와 무관하게 evidence로 남깁니다. 여기서 evidence는 모든 답변 원문 장기 저장이 아니라, 사건 판단에 필요한 사용자 발화 일부, 에이전트가 놓친 신호, 즉시 조치, 적용 범위 후보를 기록하는 자료입니다.
...
| 경로 | 역할 |
|---|---|
| `evidence/active/` | 개별 피드백, 보기 밖 선택, 보기 누락, 스킬 보고 누락, 의사소통 mismatch 중 아직 검토가 남은 evidence를 기록 |
| `evidence/applied/` | 사용자 승인 뒤 실제 규칙, 프롬프트, 스킬, project 기준에 반영한 evidence를 보관 |
| `evidence/closed/` | 반영하지 않기로 했거나 더 추적하지 않는 evidence를 보관 |
| `reports/` | 사용자가 원하는 주기로 여는 검토 세션에서 여러 evidence를 묶어 장기 반영 follow-up으로 정리한 업데이트 보고서 |
| `index.md` | evidence 목록, 상태 디렉토리, 연결된 보고서, 반영 여부를 추적 |

evidence update는 기본적으로 `evidence/active/`만 대상으로 합니다. `evidence/applied/`와 `evidence/closed/`는 명시 재검토 요청이 없으면 제외합니다.
