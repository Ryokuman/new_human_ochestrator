...
아래 신호가 보이면 0계층 전역 규칙이 아니라 1계층 Project SSoT, 2계층 Project Work SSoT, task 계약, 또는 silo local feedback 후보로 먼저 분류합니다.
...
1계층 project overview, registry, config에는 특정 task/silo의 구현 준비 상태를 올리지 않습니다. 특정 `TASK-NNNN`, 단일 API/page 실행 계약, endpoint 추가 필요성, task 고유 `Initial Mock Data`/`Test Inputs`는 2계층 Project Work SSoT의 task 문서/runbook 또는 사일로 `goal.md`로 내려보내고, 1계층에는 정본 위치와 하위 SSoT 인덱스만 둡니다.

## 계층 라우팅

| 발견한 내용 | system에 남길 것 | 내려보낼 위치 |
|---|---|---|
| 외부 서비스나 인증 provider별 동작 | 통제 가능성, 증명 가능성, 사용자 승인 필요 여부 | 1계층 Project SSoT의 service/auth policy, 2계층 auth runbook, task 계약 |
...
| 화면별 버튼, 인증 전환 화면, error UI | 실제 사용자 경로와 agent 통제 경로를 분리하는 기준 | 2계층 project task, QA checklist, 화면별 acceptance |
...
| seed, fixture, mock, 개발용 인증, 컨테이너 DB | 통제된 대체 검증과 실제 사용자 경로를 섞지 않는 기준 | 2계층 project test contract, runtime/runbook, task Test Plan |
| L 단계, runner, coverage gate | system에 고정하지 않는다는 원칙 | 2계층 Project Work SSoT의 L 기준, run set, QA gate |
| task dashboard, issue board, viewer/plugin 실행 설정 | 1계층 정본 기준과 2계층 작업 실행 기준을 섞지 않는 원칙 | 2계층 Project Work SSoT dashboard/runbook 또는 해당 task |
| 특정 task에서 반복 발견된 문제 | feedback/follow-up 판단 기준 | 2계층 project issue/task 또는 silo PR 본문 |
...
4. 실제 실행 처방은 project-level contract/registry와 분리해 성격에 따라 1계층 Project SSoT 반복 기준, 2계층 Project Work SSoT, task 계약, issue, runbook, QA checklist 중 하나로 내려보냅니다.
...
7. project-level 정본 위치, Project Work SSoT 위치, 또는 기준 브랜치가 불명확하면 system에 임시 처방을 쓰지 않고 `project-level 정본 또는 Project Work SSoT 위치 누락` 또는 `task 계약 누락`으로 보고합니다.
...
1계층 Project SSoT로 내려보낼 project 정본 기준
- ...

2계층 Project Work SSoT/task/issue/QA/runbook으로 내려보낼 실행 처방
- ...

현재 PR에서 처리하지 않고 feedback/follow-up으로 남길 항목
- ...

누락된 project-level 정본 또는 Project Work SSoT 정의
- ...
```

`system에 없음`은 누락을 뜻하지 않을 수 있습니다. 특정 provider, 화면, fixture, L gate가 system에 없고 1계층 Project SSoT, 2계층 Project Work SSoT, task 계약으로 라우팅되어 있다면 의도된 분리입니다.
...
- Project SSoT 또는 Project Work SSoT 위치가 불명확한 상태에서 system 문서에 임시 실행 절차를 고정하지 않습니다.
